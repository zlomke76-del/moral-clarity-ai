...
function DeterminationBadge({
  status,
}: {
  status: "pending" | "denied" | "executing";
}) {
  const config =
    status === "executing"
      ? {
          label: "Executable",
          tone:
            "border-emerald-400/28 bg-emerald-400/[0.08] text-emerald-100 shadow-[0_0_24px_rgba(16,185,129,0.18)]",
        }
      : status === "denied"
      ? {
          label: "Denied",
          tone:
            "border-rose-400/28 bg-rose-400/[0.08] text-rose-100 shadow-[0_0_24px_rgba(244,63,94,0.16)]",
        }
      : {
          label: "Evaluating",
          tone:
            "border-cyan-300/22 bg-cyan-400/[0.08] text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,0.14)]",
        };

  return (
    <div
      className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.24em] ${config.tone}`}
    >
...
function AnimatedCandidateNode({
  title,
  subtitle,
  status,
  delay = "0s",
  ...
  title: string;
  subtitle: string;
  status: "pending" | "rejected" | "survivor";
  delay?: string;
}) {
  const tone =
    status === "survivor"
      ? "border-cyan-300/34 bg-cyan-400/[0.10] text-white shadow-[0_0_34px_rgba(34,211,238,0.14)]"
      : status === "rejected"
      ? "border-white/8 bg-white/[0.02] text-slate-500 opacity-45"
      : "border-white/12 bg-white/[0.04] text-slate-300";

  const extra =
    status === "survivor"
      ? "solace-node-survivor"
      : status === "rejected"
      ? "solace-node-reject"
      : "solace-node-pending";
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border px-4 py-3 transition-all duration-700 ${tone} ${extra}`}
      style={{ animationDelay: delay }}
    >
      <div className="text-sm font-semibold">{title}</div>
      <div className="mt-1 text-xs leading-5 text-slate-400">{subtitle}</div>
      {status === "survivor" ? (
        <div className="mt-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-cyan-200">
          Admissible path retained
        </div>
      ) : null}
      {status === "rejected" ? (
        <div className="mt-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-rose-200/70">
    ...
    () => [
      {
        ...
        title: "Boundary reading active",
        body: "Candidate set is being evaluated against current state, scope, and proof conditions.",
        state: "Resolving state",
        authority: "Checking scope",
        receipt: "Awaiting signed proof",
      },
      {
        a: "rejected" as const,
        b: "pending" as const,
        c: "pending" as const,
        status: "pending" as const,
        title: "Candidate A removed",
        body: "High confidence is not enough. The state is insufficient for consequence.",
        state: "Insufficient",
      ...
      },
      {
        ...
        c: "pending" as const,
        status: "pending" as const,
        title: "Candidate B removed",
        body: "Aligned reasoning still fails when the requested act is outside valid scope.",
        state: "Grounded",
      ...
      },
      {
        ...
        title: "Candidate C admitted",
        body: "The surviving path is grounded, inside scope, and bound to active proof.",
        state: "Grounded",
        authority: "In scope",
    ...
    <div className="solace-panel relative overflow-hidden rounded-[1.9rem] border border-cyan-400/15 bg-[linear-gradient(180deg,rgba(7,26,44,0.96),rgba(5,15,28,0.98))] p-5 shadow-[0_26px_90px_rgba(0,0,0,0.38),0_0_42px_rgba(34,211,238,0.06)]">
      ...
      <div className="mb-5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <DeterminationBadge status={current.status} />
      ...
      <div className="mb-5 rounded-2xl border border-cyan-300/14 bg-[#071320]/88 px-4 py-3 font-mono text-[11px] uppercase tracking-[0.18em] text-cyan-100/90">
        Trace · {current.title} · State {current.state} · Authority {current.authority} · Receipt {current.receipt}
      </div>

      <div className="grid gap-6 lg:grid-cols-[0.9fr_0.2fr_1.1fr_0.2fr_0.9fr] lg:items-center">
        <div className="space-y-3">
          <AnimatedCandidateNode
            title="Candidate A"
        ...
        <div className="hidden items-center justify-center lg:flex">
          <div className="relative h-px w-full bg-gradient-to-r from-cyan-300/0 via-cyan-300/45 to-cyan-300/0">
            <div className={`solace-beam absolute inset-y-[-5px] left-0 w-20 rounded-full bg-cyan-300/35 blur-md ${current.status === "executing" ? "opacity-100" : "opacity-70"}`} />
          </div>
        </div>
        ...
        <div className="relative overflow-visible rounded-[1.8rem] border border-cyan-300/20 bg-[linear-gradient(180deg,rgba(20,61,90,0.28),rgba(9,26,42,0.5))] p-6 shadow-[0_0_50px_rgba(34,211,238,0.08)]">
          ...
          <div className="relative">
            ...
            <div className="relative z-30 mt-8 -mb-2 flex flex-wrap gap-3 overflow-visible">
              <div
                className={`group solace-floating-chip rounded-full px-4 py-2.5 text-center text-[11px] uppercase tracking-[0.18em] text-cyan-100 transition duration-300 hover:scale-[1.04] ${current.state === "Grounded" ? "ring-1 ring-cyan-300/28" : ""}`}
              ...
              <div
                className={`group solace-floating-chip rounded-full px-4 py-2.5 text-center text-[11px] uppercase tracking-[0.18em] text-cyan-100 transition duration-300 hover:scale-[1.04] ${current.authority === "In scope" ? "ring-1 ring-cyan-300/28" : ""}`}
              ...
              <div
                className={`group solace-floating-chip rounded-full px-4 py-2.5 text-center text-[11px] uppercase tracking-[0.18em] text-cyan-100 transition duration-300 hover:scale-[1.04] ${current.receipt === "Signed and active" ? "ring-1 ring-cyan-300/28" : ""}`}
        ...
        <div className="hidden items-center justify-center lg:flex">
          <div className="relative h-px w-full bg-gradient-to-r from-cyan-300/0 via-cyan-300/45 to-cyan-300/0">
            {current.status === "executing" ? (
              <div className="solace-beam absolute inset-y-[-5px] left-0 w-24 rounded-full bg-cyan-300/40 blur-md" />
            ) : null}
          </div>
        </div>

        <div className="space-y-4">
          <div className={`rounded-[1.65rem] border p-5 transition-all duration-700 ${current.status === "executing" ? "border-cyan-300/28 bg-[linear-gradient(180deg,rgba(34,211,238,0.14),rgba(34,211,238,0.05))] shadow-[0_0_36px_rgba(34,211,238,0.10)]" : "border-cyan-400/20 bg-[linear-gradient(180deg,rgba(34,211,238,0.08),rgba(34,211,238,0.03))] shadow-[0_0_26px_rgba(34,211,238,0.05)]"}`}>
            ...
            <div className="mt-3 text-lg font-semibold text-white">
              {current.status === "executing"
                ...
                : "Awaiting a surviving path through the boundary."}
            </div>
            <div className="mt-4 space-y-2 text-sm text-slate-300">
              <div>State: {current.state}</div>
              <div>Authority: {current.authority}</div>
              <div>Receipt: {current.receipt}</div>
            </div>
            <div className="mt-4 rounded-2xl border border-cyan-300/14 bg-[#071320]/90 px-4 py-3 font-mono text-[11px] uppercase tracking-[0.18em] text-cyan-100/90">
              {current.status === "executing"
                ...
                : "Hash pending · TTL pending · Scope awaiting admissibility"}
            </div>
          </div>
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_50%_-10%,rgba(34,211,238,0.08),transparent_28%),radial-gradient(circle_at_85%_8%,rgba(59,130,246,0.08),transparent_24%),linear-gradient(180deg,#040b14_0%,#06101d_32%,#040c15_100%)] text-white">
      <style>{`
        ...
        .solace-beam {
          animation: solaceBeam 1.8s ease-in-out infinite;
        }

        .solace-node-reject {
          animation: solaceReject 1.9s ease-in-out infinite;
        }

        .solace-node-survivor {
          animation: solaceSurvive 2.8s ease-in-out infinite;
        }
