...
This matches what SolaceDock expects and keeps future turns light.

---

### Full file: `app/api/chat/route.ts`
...
if (!r.ok || !r.body) {
  throw new Error(`Solace ${r.status}: ${await r.text().catch(() => '')}`);
}
...
/**
* Scrub huge image payloads (base64 data URLs, <img> tags) from messages
* so they don't blow up token usage or model context.
*/
function scrubImageContent(msgs: any[]): any[] {
return msgs.map((m) => {
  if (!m || typeof m.content !== 'string') return m;
  let content = m.content as string;

  // Remove any base64 data URLs
  content = content.replace(
    /data:image\/[a-zA-Z0-9.+-]+;base64,[A-Za-z0-9+/=]+/g,
    '[image]'
  );

  // Strip <img ...> tags down to a placeholder
...
try {
  ...
  // Scrub image payloads before any routing / memory / persona work
  const rawMessages = Array.isArray(body?.messages) ? body.messages : [];
  const messages = scrubImageContent(rawMessages);
...
<img src="${imageUrl}"
   alt="Generated image"
   style="max-width:100%;height:auto;border-radius:12px;display:block;margin:10px auto;box-shadow:0 0 18px rgba(0,0,0,.35);" />
      `.trim();
