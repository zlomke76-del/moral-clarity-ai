...
export default function SolaceDock() {
  ...
  const { visible, x, y, setPos, filters, toggle, clearFilters } = useSolaceStore();
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <TogglePill label="Ministry" value={filters.has("abrahamic") || filters.has("ministry")}
            onToggle={() => {
              ...
              if (has) {
                ...
                useSolaceStore.getState().setFilters(next);
              }
            }}
          />
        </div>
      ...
      <div className="max-h-[320px] overflow-auto px-3 py-2 space-y-2">
        {messages.map((m, i) => (
          <div key={i} className={m.role === "user" ? "text-zinc-200" : "text-zinc-400"}>
            <span className="text-xs uppercase tracking-wide text-zinc-500 mr-2">{m.role}</span>
            {m.content}
          </div>
        ))}
      </div>
