...
export default function NeuralSidebar({ items }: Props) {
  ...
  return (
    <aside className="hidden h-full w-72 flex-shrink-0 flex-col gap-4 border-r border-cyan-400/25 bg-slate-950/90 bg-gradient-to-b from-slate-950 via-slate-900/90 to-slate-950 px-4 py-5 shadow-[18px_0_40px_rgba(15,23,42,0.95)] backdrop-blur-2xl md:flex">
      {/* Logo / Brand */}
      <div className="mb-3 flex items-center gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-cyan-400/80 bg-slate-950 shadow-[0_0_22px_rgba(34,211,238,0.85)]">
...
function ChipCard({ item }: ChipCardProps) {
  const core = (
    <div className="relative flex cursor-pointer flex-col gap-1 rounded-2xl border border-cyan-400/80 bg-slate-950/95 px-3.5 py-3 shadow-[0_0_32px_rgba(34,211,238,0.85)] transition-all duration-150 hover:-translate-y-[1px] hover:border-cyan-300 hover:shadow-[0_0_40px_rgba(34,211,238,1)]">
      {/* Circuit traces */}
      <div className="pointer-events-none absolute -left-6 top-1/2 h-px w-6 -translate-y-1/2 bg-cyan-400/70" />
      <div className="pointer-events-none absolute -right-6 top-1/2 h-px w-6 -translate-y-1/2 bg-cyan-400/50" />
