          ...
          <tbody className="[&>tr:nth-child(even)]:bg-neutral-950">
            {rows.map((r) => (
              <tr key={r.id} className="align-top">
                ...
                <td className="p-3"><span className="rounded-full px-2 py-0.5 text-xs bg-neutral-800">{r.category}</span></td>
                <td className="p-3">
                  <div className="font-medium">{r.title}</div>
                  <div className="text-neutral-400 line-clamp-2">{r.description}</div>
                </td>
                <td className="p-3"><StatusBadge value={r.status} /></td>
                <td className="p-3"><PriorityBadge value={r.priority} /></td>
...
function PriorityBadge({ value }: { value: "low" | "medium" | "high" }) {
  const cls = value === "high"
    ...
    : value === "medium" ? "bg-amber-900/40 text-amber-300" : "bg-neutral-800 text-neutral-300";
  return <span className={`rounded-full px-2 py-0.5 text-xs ${cls}`}>{value}</span>;
}
