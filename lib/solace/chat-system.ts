...
export function wantsDeepResearch(text: string): boolean {
  ...
  // Core deep-research / website-eval / UX-eval triggers
  const keywords = [
    'deep research',
    'full research',
    'full analysis',
    ...
    // NEW: reviews / ratings / Google reviews
    'google reviews',
    'reviews on google',
    'google rating',
    'star rating',
  ...
  // NEW: heuristic patterns for "look up X reviews", "check X rating" etc.
  if (
    /look\s+up\s+.+reviews?/.test(t) ||
    /find\s+.+reviews?/.test(t) ||
    /check\s+.+reviews?/.test(t) ||
    /check\s+.+rating/.test(t) ||
    /what\s+do\s+people\s+say\s+about/.test(t)
  ) {
    return true;
  }

  // Any explicit URL still counts as deep research (site evaluation / content fetch)
...
- When a WEB CONTEXT, RESEARCH CONTEXT, or NEWS CONTEXT is present, rely on it for post-cutoff events.
- Never state that the current year is earlier than ${year}; that would be drift.`;
  ...
  const domain: SolaceDomain =
    wantsGuidance ? 'guidance' : wantsAbrahamic ? 'ministry' : 'core';
